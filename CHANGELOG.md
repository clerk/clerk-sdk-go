...
- Add support for sending transactional emails with the new `email` package (`email.Send`). The recipient (`SendParams.To`, a `Recipient`) accepts either an `Address` or a `UserID`; the two are mutually exclusive, and when `UserID` is set Clerk resolves that user's primary email address server-side. Responses expose `SuppressionReason` when Clerk declines delivery.
